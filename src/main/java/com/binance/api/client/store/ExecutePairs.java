...
public class ExecutePairs {
	...
	public static void main(String[] args) {
		...
		if (args.length == 1 && Arrays.asList("BTC", "BNB").contains(args[0])) {
			...
			CachePrice objCache = new CachePrice(args[0]);
			objCache.cachePrice();
			
            /*
			String pair = "BTC";
			List<String> symbols = Arrays.asList("ETHBTC", "PPTBTC", "BNBBTC", "LINKBTC", "XRPBTC", "PPTBTC", "CVCBTC");
			for (String symbol : symbols) {
				DepthCacheOrderBook dco = new DepthCacheOrderBook(pair, symbol);
				dco.start();

				CachePrice objCache = new CachePrice(symbol);
				objCache.start();
			}
			*/
		} else {
